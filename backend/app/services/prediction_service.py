        ...
        async with _lock:
            if _ensemble is None:
                try:
                    ...
                    model_path = settings.OUTPUT_BASE + "/current" + "/stocksense-qwen"
                    ...
                    logger.info(f"EnsemblePredictor initialized — loaded: {_ensemble.is_loaded}")
...
async def reload_models() -> dict:
    ...
    _ensemble = None
    predictor = await get_predictor()
    return {"loaded": predictor.is_loaded} if predictor else {"error": "reload failed"}
...
async def get_model_status() -> dict:
    ...
    if predictor:
        return {
            "qwen_loaded": predictor.is_loaded,
